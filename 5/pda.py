...
class PDA(Generic[ET]):  # Probabilistic Data Association
    ...
    def predict(self, filter_state: ET, Ts: float) -> ET:
        ...
        ET=self.state_filter.predict(ET,Ts)
        return ET
    ...
    def gate(
        ...
        *,
        ...
        for i in range(len(Z)):
            within_gate=self.state_filter.gate(Z[i],ET,g_squared,sensor_state)
            gated.append(within_gate)
        return gated
    ...
    def loglikelihood_ratios(
        ...
        *,
        ...
        for i in range(1,len(ll)):
            ll[i]=self.state_filter.loglikelihood(Z[i],ET,sensor_state=sensor_state)
